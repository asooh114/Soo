void ana(){
  ...
  double lumi = 41529;
  TString filename = "SingleElectron_Run2017";
  bool data = true;
  ...
  TFile * output = new TFile(Form("histo1_%s.root", filename.Data()),"recreate");
...
//  TH1F * h_MET_step2 = new TH1F("h_MET_step2","h_MET_step2",120,0,120);
  ...
  while(theReader.Next()){
    ...
    for(int iJet=0; iJet<jet_pt.GetSize(); ++iJet){
      if (jet_pt[iJet] > 30 && abs(jet_eta[iJet]) < 2.1) njet++;
